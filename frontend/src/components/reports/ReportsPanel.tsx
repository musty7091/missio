...
function ManagementCloseDayPanel({
  role,
  businessId,
  onOpenTaskDetails,
}: {
  ...
  const closureHasIssues = hasTasksForClosure && !canCleanClose
  ...
  return (
    <section className="flex flex-1 flex-col pb-24">
      ...
      >
        ...
        <div className="mt-2 rounded-2xl bg-white/10 px-3 py-3">
          {isDayAlreadyClosed && todayClosure ? (
            <div
              className={`rounded-2xl border p-3 text-sm font-bold leading-6 ${getClosureStatusClassName(
                todayClosure.status,
              )}`}
            >
              ...
              {todayClosure.manager_note && (
                <p className="mt-2 rounded-xl bg-white/70 px-3 py-2 text-xs dark:bg-white/10">
                  {todayClosure.manager_note}
                </p>
              )}
            </div>
