    ...
    <section className="mb-3 rounded-[1.7rem] bg-slate-950 p-4 text-white shadow-xl shadow-slate-950/15">
      <div className="mb-3 flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 rounded-2xl bg-cyan-300 px-3 py-2 text-center text-slate-950">
          ...
          <p className="mt-1 text-[0.62rem] font-black">
            {t("task.summary.completedRate")}
          </p>
        </div>
      </div>
      ...
      <div className="mb-3">
        <div className="mb-1.5 flex items-center justify-between text-[0.68rem] font-black text-slate-300">
          <span>{t("task.summary.remainingTask")}</span>
      ...
      <div className="grid grid-cols-4 gap-2">
        <SummaryMiniItem label={t("task.summary.completed")} value={completedCount} />
        <SummaryMiniItem label={t("task.summary.active")} value={activeCount} />
        <SummaryMiniItem label={t("task.summary.waiting")} value={waitingCount} />
        <SummaryMiniItem
          label={t("task.summary.rejected")}
          value={rejectedCount}
          tone="danger"
        />
      </div>
